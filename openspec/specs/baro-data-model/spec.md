...
Canonical spec (merged from change `check-structure`). See `packages/database/prisma/schema.prisma` and `/v1/baro/*` API.
...
The shared schema MUST include baro domain entities: Professional, ProfessionalRegistration, Expediente, ExpedienteColindante, ExpedienteColindanteNomenclatura, ExpedienteOrdenante, ExpedienteLinderos, ExpedienteLinderoPunto, ExpedienteActuante, ExpedienteTituloRelacion, and related enums (mapped as `Baro*` models with `@@map("baro_*")`).
