...
| Suite | Result |
|-------|--------|
| `pnpm turbo run build --filter=@hubilee/hub...` | **PASS** (standalone artifact at `apps/hub/.next/standalone/apps/hub/server.js`) |
| `docker build -f docker/Dockerfile.nextjs` (hub) | **PASS** |
| `docker run hubilee-hub-test` → `curl /` | **200** (Next.js 16.2.3 ready, no `next: not found`) |
...
- Balance compose service uses generic Dockerfile; `@hubilee/balance` remains out of product scope (not in tracked workspace).
