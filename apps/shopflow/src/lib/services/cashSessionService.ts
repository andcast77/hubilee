...
export async function listCashRegisters(storeId?: string | null): Promise<CashRegister[]> {
  ...
  const response = await shopflowApi.get<ApiResult<{ registers: CashRegister[] }>>(
...
export async function createCashRegister(data: CreateCashRegisterInput): Promise<CashRegister> {
  const response = await shopflowApi.post<ApiResult<CashRegister>>('/cash-registers', data)
...
export async function listCashSessions(params?: {
  ...
  const response = await shopflowApi.get<ApiResult<{ sessions: CashSession[] }>>(
    `/cash-sessions${qs ? `?${qs}` : ''}`
  )
...
export async function openCashSession(data: OpenCashSessionInput): Promise<CashSession> {
  const response = await shopflowApi.post<ApiResult<CashSession>>('/cash-sessions/open', data)
...
export async function closeCashSession(id: string, data: CloseCashSessionInput): Promise<CashSession> {
  const response = await shopflowApi.post<ApiResult<CashSession>>(`/cash-sessions/${id}/close`, data)
...
export async function getCashSessionReport(id: string): Promise<CashSessionReport> {
  const response = await shopflowApi.get<ApiResult<CashSessionReport>>(`/cash-sessions/${id}/report`)
