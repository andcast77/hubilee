...
export async function getSales(query: SaleQueryInput = { page: 1, limit: 20 }) {
  ...
  const response = await posApi.get<ApiResult<{ sales: any[]; pagination: any }>>(
...
export async function getSaleById(id: string) {
  const response = await posApi.get<{ success: boolean; data: any; error?: string }>(
    `/sales/${id}`
  )
...
export async function createSale(userId: string, data: CreateSaleInput) {
  ...
  const response = await posApi.post<ApiResult<any>>(
    '/sales',
    {
...
export async function settleSale(id: string, data: SettleSaleInput) {
  const response = await posApi.post<ApiResult<any>>(`/sales/${id}/settle`, {
...
export async function cancelSale(id: string) {
  ...
  const response = await posApi.post<{ success: boolean; data: any; error?: string }>(
    `/sales/${id}/cancel`,
    {}
  )
