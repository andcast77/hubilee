...
import { authApi, hrApi } from '@/lib/api/client';
...
const Sidebar: React.FC<SidebarProps> = ({ children }) => {
  ...
  const userWithCompany = user as { company?: { hrEnabled?: boolean }; isSuperuser?: boolean };
  if (
    userWithCompany.company?.hrEnabled === false &&
      ...
      <div className="min-h-screen flex items-center justify-center p-4 bg-amber-50">
        <div className="max-w-md text-center">
          ...
          <p className="text-amber-800">
            El módulo Hr no está activado para esta empresa. Contacta al administrador para activarlo.
          </p>
        </div>
      </div>
    );
