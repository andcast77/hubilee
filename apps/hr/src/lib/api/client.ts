...
// Hr API: prefix /v1/hr (central API)
export const hrApi = {
  get: <T>(endpoint: string, options?: RequestInit) =>
    apiClient.get<T>(`/v1/hr${endpoint.startsWith('/') ? endpoint : `/${endpoint}`}`, options),
  post: <T>(endpoint: string, data?: unknown, options?: RequestInit) =>
    apiClient.post<T>(`/v1/hr${endpoint.startsWith('/') ? endpoint : `/${endpoint}`}`, data, options),
  put: <T>(endpoint: string, data?: unknown, options?: RequestInit) =>
    apiClient.put<T>(`/v1/hr${endpoint.startsWith('/') ? endpoint : `/${endpoint}`}`, data, options),
  delete: <T>(endpoint: string, data?: unknown, options?: RequestInit) =>
    apiClient.delete<T>(`/v1/hr${endpoint.startsWith('/') ? endpoint : `/${endpoint}`}`, data, options),
}
