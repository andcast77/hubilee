...
3. El `vercel.json` de `apps/api` define **`buildCommand`** → `cd ../.. && pnpm exec turbo run build --filter=@hubilee/api...`, la función `api/index.ts` (con `includeFiles` de `dist/**` + Prisma generated), los rewrites y los crons. El Prisma Client se genera bajo `apps/api/src/generated/prisma` (`pnpm --filter @hubilee/api db:generate` / `prisma generate`).
