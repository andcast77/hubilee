...
│   ├── swagger.ts
│   ├── db/                   # Acceso Prisma
│   ├── generated/prisma/     # Prisma Client (local `prisma generate`; gitignored)
...
### Vercel (monorepo, Root Directory = `apps/api`)

1. **Root Directory**: deja **Root Directory** = `apps/api` en el proyecto de Vercel.
2. **Incluir fuentes fuera del Root Directory**: en el proyecto Vercel → **Settings** → **General** → **Root Directory** → activa **"Include source files outside of the Root Directory"**. Así el build desde la raíz del repo (`cd ../.. && pnpm exec turbo run build --filter=@hubilee/api...`) puede resolver el workspace monorepo.
3. El `vercel.json` de `apps/api` define **`framework: null`** (evita que Vercel monte un entry Fastify desde `src/` con imports ESM sin `.js`), **`buildCommand`** → contracts + `pnpm --filter @hubilee/api run build:vercel` (Prisma generate + esbuild `dist/vercel-server.js`), la función `api/index.ts` (con `includeFiles` del bundle), rewrites `/` + `/:path*` → `/api`, y los crons. El Prisma Client se genera bajo `apps/api/src/generated/prisma`.
...
6. **Base de datos ya cifrada**: si ya existen filas cifradas con una clave anterior, usa **esa misma** `FIELD_ENCRYPTION_KEY` en Vercel. Cambiar la clave sin re-cifrar rompe la lectura; ver `apps/api/scripts/rotate-field-key.ts` y el doc anterior.
