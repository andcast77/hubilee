...
3. El `vercel.json` de `apps/api` define **`buildCommand`** → contracts + `pnpm --filter @hubilee/api run build:vercel` (Prisma generate + esbuild `dist/vercel-server.js`), la función `api/index.ts` (con `includeFiles` del bundle), los rewrites y los crons. El Prisma Client se genera bajo `apps/api/src/generated/prisma`.
