...
export async function getSaleById(
  ...
  // deny too, not be treated as "no restriction" (see pos-authorization.policy.ts).
...
export async function createSale(
  ...
  if (isDirect) {
    // The route only requires `pos.sales:create`. Sending a cashSessionId settles the
    // sale inline as COMPLETED (a privileged settle), so this path must ALSO require
    // `pos.sales:settle` — otherwise a create-only role (Vendedor) could bypass the
...
export async function refundSale(
  ...
  // `pos.sales:refund` via requirePermission, but refund is a money-out, elevated action
  ...
  // another entry point.
  await assertPermission(ctx, 'pos.sales', 'refund', 'No tienes permiso para reembolsar ventas')
