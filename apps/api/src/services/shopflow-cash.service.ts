...
export async function createCashRegister(ctx: ShopflowContext, input: CreateCashRegisterInput): Promise<CashRegisterRow> {
...
export async function listCashRegisters(ctx: ShopflowContext, query: ListCashRegistersQuery): Promise<CashRegisterRow[]> {
...
export async function openCashSession(ctx: ShopflowContext, input: OpenCashSessionInput): Promise<CashSessionRow> {
  const cash = createRepositories(ctx.companyId).cash
...
export async function listCashSessions(ctx: ShopflowContext, query: ListCashSessionsQuery): Promise<CashSessionRow[]> {
  const effectiveStoreId = await resolveEffectiveStoreIdForScopedUser(ctx, query.storeId)
  const cash = createRepositories(ctx.companyId).cash
...
export async function getCashSessionReport(ctx: ShopflowContext, id: string): Promise<CashSessionReport> {
  const cash = createRepositories(ctx.companyId).cash
