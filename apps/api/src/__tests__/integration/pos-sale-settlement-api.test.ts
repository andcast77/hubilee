 ...
 * Covers `requirePermission('pos.sales', 'settle')` RBAC gating on
 * `POST /v1/pos/sales/:id/settle`, plus the PENDING creation + list flow.
 *
 * `pos.sales.settle` is granted to a locally-seeded Cajero-shaped role here
 * (mirrors pos-cash-session-api.test.ts's PR2 pattern) — the global seed.ts
...
describe('Pos sale settlement: HTTP API + RBAC', () => {
  ...
  it('creates a PENDING sale without invoice number, then lists it via status=PENDING', async () => {
    ...
    const { res, json } = await inject(app, {
      method: 'GET',
      url: '/v1/pos/sales?status=PENDING',
  ...
  it('vendedor (no settle permission) gets 403 settling a pending sale', async () => {
    ...
    const { res } = await inject(app, {
      method: 'POST',
      url: `/v1/pos/sales/${saleId}/settle`,
  ...
  // FIX 2 (pos-cash-session): POST /sales only requires `pos.sales:create` at the
  ...
  // `pos.sales:settle` by taking this path instead of POST /sales/:id/settle.
