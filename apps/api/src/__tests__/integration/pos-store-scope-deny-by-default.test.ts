 ...
 *
 * Root cause (fixed here): `requirePosContext` used to set `request.storeId =
 ...
 * therefore bypass store isolation on every mutating + read pos endpoint simply by
...
describe('Pos store scope: deny-by-default when X-Store-Id header is omitted', () => {
  ...
  beforeAll(async () => {
    ...
    // granted every pos permission this test exercises so failures are attributable
    ...
    const permissions = await Promise.all([
      upsertPermission('pos.sales.create', 'pos.sales', 'create'),
      upsertPermission('pos.sales.cancel', 'pos.sales', 'cancel'),
      upsertPermission('pos.sales.settle', 'pos.sales', 'settle'),
      upsertPermission('pos.sales.refund', 'pos.sales', 'refund'),
      upsertPermission('pos.cash-registers.create', 'pos.cash-registers', 'create'),
      upsertPermission('pos.cash-registers.read', 'pos.cash-registers', 'read'),
      upsertPermission('pos.cash-sessions.open', 'pos.cash-sessions', 'open'),
      upsertPermission('pos.cash-sessions.close', 'pos.cash-sessions', 'close'),
      upsertPermission('pos.cash-sessions.read', 'pos.cash-sessions', 'read'),
    ])
  ...
  it('cancelSale: denied cross-store without X-Store-Id header', async () => {
    ...
    const { res } = await inject(app, {
      method: 'POST',
      url: `/v1/pos/sales/${sale.id}/cancel`,
  ...
  it('refundSale: denied cross-store without X-Store-Id header', async () => {
    ...
    const { res } = await inject(app, {
      method: 'POST',
      url: `/v1/pos/sales/${sale.id}/refund`,
      headers: { Authorization: `Bearer ${scopedToken}`, 'content-type': 'application/json' },
      payload: {},
    })
    expect(res.statusCode).toBe(403)
  ...
  it('settleSale: denied cross-store without X-Store-Id header', async () => {
    ...
    const { res } = await inject(app, {
      method: 'POST',
      url: `/v1/pos/sales/${sale.id}/settle`,
      headers: { Authorization: `Bearer ${scopedToken}`, 'content-type': 'application/json' },
  ...
  it('closeCashSession: denied cross-store session without X-Store-Id header', async () => {
    ...
    const { res } = await inject(app, {
      method: 'POST',
      url: `/v1/pos/cash-sessions/${session.id}/close`,
      headers: { Authorization: `Bearer ${scopedToken}`, 'content-type': 'application/json' },
  ...
  it('listSales: denied when query storeId targets a store outside the derived scope', async () => {
    ...
    const { res } = await inject(app, {
      method: 'GET',
      url: `/v1/pos/sales?storeId=${storeBId}`,
  ...
  it('listCashRegisters: denied when query storeId targets a store outside the derived scope', async () => {
    const { res } = await inject(app, {
      method: 'GET',
      url: `/v1/pos/cash-registers?storeId=${storeBId}`,
      headers: { Authorization: `Bearer ${scopedToken}` },
    })
    expect(res.statusCode).toBe(403)
  })
  ...
  it('listCashSessions: denied when query storeId targets a store outside the derived scope', async () => {
    const { res } = await inject(app, {
      method: 'GET',
      url: `/v1/pos/cash-sessions?storeId=${storeBId}`,
      headers: { Authorization: `Bearer ${scopedToken}` },
    })
    expect(res.statusCode).toBe(403)
  })
