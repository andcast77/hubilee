/**
 * POS Cash Session (Round 3, FIX 2): `POST /v1/pos/sales/:id/refund` used to have only
 * `preHandler: pre` — no permission check at all, and `pos.sales.refund` didn't even exist
 * as a permission. Any pos-module user (e.g. Vendedor) could refund any COMPLETED sale and
 * silently drop it from an open session's arqueo. Refund is now gated on
 * `requirePermission('pos.sales', 'refund')`, granted only to elevated roles (Gerente;
...
describe('Pos sale refund: HTTP API + RBAC (Round 3, FIX 2)', () => {
  ...
  beforeAll(async () => {
    ...
    const createPerm = await upsertPermission('pos.sales.create', 'pos.sales', 'create')
    ...
    const settlePerm = await upsertPermission('pos.sales.settle', 'pos.sales', 'settle')
    const cancelPerm = await upsertPermission('pos.sales.cancel', 'pos.sales', 'cancel')
  ...
  it('Vendedor (create-only) gets 403 refunding a COMPLETED sale', async () => {
    ...
    const { res } = await inject(app, {
      method: 'POST',
      url: `/v1/pos/sales/${saleId}/refund`,
  ...
  it('Gerente (real seeded role, has pos.sales.refund) can refund a COMPLETED sale', async () => {
    const saleId = await createCompletedSale()
