 ...
 * Exercises `services/pos-sales.service.ts` (createSale two flows + settleSale)
 * directly — no HTTP layer (that's covered by pos-cash-session-api.test.ts style
...
import * as salesService from '../../services/pos-sales.service.js'
...
describe('Pos sale lifecycle + settlement (PR4)', () => {
  ...
  // real seeded Gerente role, which now carries `pos.sales.refund` (FIX 2's seed grant),
