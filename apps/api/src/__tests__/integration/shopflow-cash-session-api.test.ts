 ...
 * Covers `requirePermission('shopflow.cash-registers'|'shopflow.cash-sessions', action)`
 * RBAC gating, tenant isolation, and store scoping on top of shopflow-cash.service.ts.
 *
 * Permissions/roles used here (shopflow.cash-registers.*, shopflow.cash-sessions.*) are
...
describe('Shopflow cash registers/sessions: HTTP API + RBAC', () => {
  ...
  beforeAll(async () => {
    ...
    const openPerm = await upsertPermission('shopflow.cash-sessions.open', 'shopflow.cash-sessions', 'open')
    const closePerm = await upsertPermission('shopflow.cash-sessions.close', 'shopflow.cash-sessions', 'close')
    const readPerm = await upsertPermission('shopflow.cash-sessions.read', 'shopflow.cash-sessions', 'read')
  ...
  it('OWNER lists cash registers scoped to a store', async () => {
    ...
    const { res, json } = await inject(app, {
      method: 'GET',
      url: `/v1/shopflow/cash-registers?storeId=${acmeStoreId}`,
  ...
  it('full end-to-end: open -> settle sale via direct DB write -> close with arqueo -> report', async () => {
    ...
    const report = await inject(app, {
      method: 'GET',
      url: `/v1/shopflow/cash-sessions/${sessionId}/report`,
      headers: { Authorization: `Bearer ${ownerToken}` },
    })
    ...
    const closed = await inject(app, {
      method: 'POST',
      url: `/v1/shopflow/cash-sessions/${sessionId}/close`,
      headers: { Authorization: `Bearer ${ownerToken}`, 'content-type': 'application/json' },
  ...
  it('cajero (locally-granted cash-sessions perms) can open and close a session', async () => {
    ...
    const closed = await inject(app, {
      method: 'POST',
      url: `/v1/shopflow/cash-sessions/${opened.json.data.id}/close`,
      headers: { Authorization: `Bearer ${cajeroToken}`, 'content-type': 'application/json', 'x-store-id': acmeStoreId },
